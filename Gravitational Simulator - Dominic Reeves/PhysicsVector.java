...
public class PhysicsVector {
	...
	* @return A scaled vector quantity.
	**/
	...
	/**
	* scale the components of each vector in an array by a scalar quantity.
	*
	* @param x the factor to scale the vector by.  
	* @param v the vector to scale
	* @param N The number of vectors in the array.
	* @return The array of scaled particles.
	**/
	public static PhysicsVector[] scaleArray(double x, PhysicsVector[] v, int N){
		PhysicsVector[] Scaled = new PhysicsVector[N];
		for (int j=0; j<N;j++){
			Scaled[j] = new PhysicsVector(v[j]);
		}
		for (int j=0; j<N;j++){
			for (int i=0; i<v[j].vectorComponents.length; i++) {
				Scaled[j].vectorComponents[i] *=x;
			}
		}
		return Scaled;
	}
	...
	**/
	...
	/**
	* cross is the standard vector product, combining elements 
	*
	* @param v first vector in product
	* @param u second vector in product
	...
	**/
	public static PhysicsVector vectorProduct(PhysicsVector v, PhysicsVector u){
		...
		product.vectorComponents[0] = v.vectorComponents[1]*u.vectorComponents[2] - v.vectorComponents[2]*u.vectorComponents[1];
		product.vectorComponents[1] = v.vectorComponents[2]*u.vectorComponents[0]-v.vectorComponents[0]*u.vectorComponents[2];
		product.vectorComponents[2] =  v.vectorComponents[0]*u.vectorComponents[1] - v.vectorComponents[1]*u.vectorComponents[0];
		return product;
	}
	...
	**/
	...
	/**
	* Standard vector addition of each vector in an array by the corresponding vector in another
	*
	* @param v first vector array in sum
	* @param u second vector array in sum
	* @param N The size of each array of vectors
	* @return return summed vector array
	**/
	public static PhysicsVector[] addArray(PhysicsVector[] v, PhysicsVector[] u, int N){
		PhysicsVector[] Sum = new PhysicsVector[N];
		for(int i = 0; i < N; i++){
			Sum[i] = new PhysicsVector(v[i]);
		}
		for(int i = 0; i < N; i++){
			Sum[i].increaseBy(u[i]);
		}
		return Sum;
	}
